"""Code taken from https://github.com/tambetm/simple_dqn/blob/master/src/replay_memory.py"""
...
from uitls.util import print_and_log_message, print_and_log_message_list

class ReplayMemory:
    def __init__(self, logger, config):
        self.logger = logger

        self.model_dir = join(config[SAVE_DIR], REPLAY_MEMORY)
        ...
        self.screens = np.empty((self.memory_size, config[NUM_CHANNELS]), dtype = np.floa32)
    ...
    def add(self, screen, reward, action, terminal):
        assert screen.shape == self.num_channels
    ...
    def save(self):
        for idx, (name, array) in enumerate(
            ...
            save_npy(array, join(self.model_dir, name))

    def load(self):
        for idx, (name, array) in enumerate(
            zip([ACTIONS, REWARDS, SCREENS, TERMINALS, PRESTATES, POSTSTATES],
                [self.actions, self.rewards, self.screens, self.terminals, self.prestates, self.poststates])):
            array = load_npy(join(self.model_dir, name))
